 ...
 *like OOPs again debugfs/dmesg/octaldump may help. I personally like attaching the JTAG to the embedded hw and
 *put the breakpoint and look at all the stack traces, register dumps like ( PC, STACK PTR, CPU status registers ) etc.
 *If the crash is random you may want to look into the JTAG ETM and collect the last executed instruction sets etc.
 *It may be related to the HW as well. In that case try changing the kernel to single core, reduced clock regime etc.
 */
