/*
 * FindSmallestPositiveIntegerThatCannotBeRepresentedAsSumOfAnySubsetOfGivenArray.cpp
 *
...
A Simple Solution is to start from value 1 and check all values one by one if they can sum to values in
the given array. This solution is very inefficient as it reduces to subset sum problem which is a well
...
arr[i] (why? If elements from 0 to (i-1) can represent 1 to ‘res-1′, then elements from 0 to i can
represent from 1 to ‘res + arr[i] – 1′ be adding ‘arr[i]‘ to all subsets that represent 1 to ‘res’)
 */
