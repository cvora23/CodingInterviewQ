 ...
 * at arr[i+1] or arr[i-1]. Write an efficient function to search an element in this array.
 * Basically the element arr[i] can only be swapped with either arr[i+1] or arr[i-1].
 * For example consider the array {2, 3, 10, 4, 40}, 4 is moved to next position and
 * 10 is moved to previous position.
 * Example:
 ...
 * If not present, then compare the key with middle element to decide whether to go in left
 * half or right half. Comparing with middle element is enough as all the elements after
 * mid+2 must be greater than element mid and all elements before mid-2 must be smaller than mid element.
