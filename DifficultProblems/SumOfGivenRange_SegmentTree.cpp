/*
 * SumOfGivenRange_SegmentTree.cpp
 *
...
An array representation of tree is used to represent Segment Trees. For each node at index i, the left child is at index 2*i+1,
right child at 2*i+2 and the parent is at (i-1)/2.

http://www.geeksforgeeks.org/wp-content/uploads/segment-tree1.png
...
Following is algorithm to get the sum of elements.
...
The time complexity of update is also O(Logn). To update a leaf value, we process one node at every level
