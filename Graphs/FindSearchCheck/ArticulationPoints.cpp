 ...
 * The idea is to use DFS (Depth First Search). In DFS, we follow vertices in tree form
 * called DFS tree. In DFS tree, a vertex u is parent of another vertex v, if v is discovered
 * by u (obviously v is an adjacent of u in graph). In DFS tree, a vertex u is articulation
 ...
 * For every vertex, count children. If currently visited vertex u is root (parent[u] is NIL)
 * and has more than two children, print it.
 * How to handle second case? The second case is trickier. We maintain an array disc[]
 * to store discovery time of vertices. For every node u, we need to find out
 * the earliest visited vertex (the vertex with minimum discovery time) that can be reached
...
// Low[u ] = lowest numbered vertex reachable from u using 0 or more spanning tree edges and then at most one back edge
    ...
    for (i = adj[u].begin(); i != adj[u].end(); ++i)
    {
        ...
        if (!visited[v]) // Forward edge
        {
            ...
            if (parent[u] == NIL && children > 1){
            	cout<<"Satisying rule 1 : "<<u<<endl;
               ap[u] = true;
            }

            // (2) If u is not root and low value of one of its child is more
            // than discovery value of u.
            // I.e., is there a child v of u that cannot reach a vertex visited before u?
