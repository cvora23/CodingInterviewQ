 ...
 *  in the graph. A back edge is an edge that is from a node to itself (selfloop) or one of
 *  its ancestor in the tree produced by DFS. In the following graph, there are 3 back edges,
 *  marked with cross sign. We can observe that these 3 back edges indicate 3 cycles present
 *  in the graph.
 *  For a disconnected graph, we get the DFS forrest as output. To detect cycle, we can
 *  check for cycle in individual trees by checking back edges.
 *
